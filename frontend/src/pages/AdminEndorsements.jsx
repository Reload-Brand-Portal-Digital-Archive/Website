...
export default function AdminEndorsements() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.name) return notify.error("Nama wajib diisi!");
        if (selectedEndorsement === null && !formData.image) return notify.error("Gambar wajib diupload!");
        ...
        try {
            ...
            notify.update(loadingToastId, { render: res.data.message || (selectedEndorsement ? "Endorsement berhasil diperbarui!" : "Endorsement berhasil ditambahkan!"), type: 'success', isLoading: false, autoClose: 3000 });
            setEndorsements(res.data.data);
            handleCancelEdit();
        } catch (error) {
            notify.error(error.response?.data?.message || (selectedEndorsement ? "Gagal memperbarui endorsement" : "Gagal menambah endorsement"));
        }
    };
    ...
    const handleDelete = async (id, name) => {
        const confirmed = await confirm({
            ...
            description: `Yakin ingin menghapus endorsement "${name}"? Tindakan ini tidak dapat dikembalikan!`,
            confirmText: 'Hapus',
            cancelText: 'Batal',
        });
        ...
        if (confirmed) {
            try {
                ...
                notify.update(loadingToastId, { render: res.data.message || 'Endorsement berhasil dihapus!', type: 'success', isLoading: false, autoClose: 3000 });
            } catch (error) {
                const errorMessage = error.response?.data?.message || "Gagal menghapus endorsement";
    ...
    const handleToggleActive = async (endorsement) => {
        ...
        try {
            ...
            setEndorsements(res.data.data);
            notify.success(`Endorsement ${newActiveState ? 'diaktifkan' : 'dinonaktifkan'}`);
        } catch (error) {
            notify.error("Gagal mengubah status endorsement");
        }
    };
            ...
            {currentView === 'list' ? (
                <>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            ...
                            <p className="text-sm text-zinc-400 mt-1">Kelola social proof / endorsement yang ditampilkan di halaman depan.</p>
                        </div>
                        <button onClick={() => { setCurrentView('form'); setSelectedEndorsement(null); setFormData({ name: '', image: null, is_active: true }); setPreview(null); }} className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white rounded-md font-medium flex items-center gap-2 transition-colors">
                            <Plus size={18} /> Tambah Endorsement
                        </button>
                    </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                            {endorsements.length === 0 ? (
                                <div className="col-span-full bg-zinc-900 border border-zinc-800 rounded-lg p-12 text-center">
                                    ...
                                    <h3 className="text-lg font-medium text-zinc-300">Belum ada endorsement</h3>
                                    <p className="text-zinc-500 mt-1">Tambahkan endorsement pertama untuk ditampilkan di Landing Page.</p>
                                </div>
                            ) : (
                                    ...
                                    return (
                                        <div key={item.id} className={`bg-zinc-900 border rounded-lg overflow-hidden group hover:border-zinc-600 transition-colors ${isActive ? 'border-zinc-800' : 'border-zinc-800/50 opacity-60'}`}>
                                            <div className="aspect-square bg-zinc-950 relative border-b border-zinc-800">
                                                ...
                                                {!isActive && (
                                                    <span className="absolute top-2 left-2 bg-zinc-800/90 text-zinc-400 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">Nonaktif</span>
                                                )}
                                            </div>
                ...
                <>
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-bold text-zinc-50 flex items-center gap-2"><Award className="text-rose-500" /> {selectedEndorsement ? 'Edit Endorsement' : 'Tambah Endorsement Baru'}</h2>
                            <p className="text-sm text-zinc-400 mt-1">Atur social proof untuk halaman depan.</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 p-6 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="col-span-2 space-y-4">
                            <div>
                                <label className="text-sm text-zinc-400 mb-1 block">Nama Endorser</label>
                                <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="Ex: John Doe / @username" className="w-full bg-zinc-950 border border-zinc-800 rounded-md py-2 px-4 text-zinc-100 focus:outline-none focus:border-rose-500 transition-colors" />
                            </div>
                            <div className="flex items-center gap-3 py-2">
                                <label className="text-sm text-zinc-400">Status Aktif</label>
                                <button
                                    type="button"
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            <div className="flex gap-2">
                                <button type="submit" className="flex-1 bg-rose-500 hover:bg-rose-600 text-white py-2 rounded-md font-medium flex items-center justify-center gap-2 transition-colors"><Plus size={18} /> {selectedEndorsement ? 'Update' : 'Simpan'}</button>
